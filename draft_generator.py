...
# Spec: draft every project scoring >= this, regardless of whether an
# Updates-sheet event exists for it yet. This is the main fix for the
# "Drafts sheet remains empty" problem — V2 only ever drafted off of
# Updates-sheet rows, so if update_tracker found nothing, draft_generator
# had nothing to work with even for obviously good, high-scoring projects.
DRAFT_MIN_SCORE = getattr(config, "NOTIFY_MIN_SCORE", 7.0)
HIGH_CONFIDENCE_MIN = getattr(config, "HIGH_CONFIDENCE_MIN", 70)
...
def run_generator() -> None:
    ...
    # ── Direct score-based pass (spec: draft every project Score >= 7,
    # or a high-confidence project with an important update — the loop
    # above already covers the update-driven half of that; this covers
    # projects that qualify on score alone, even with zero tracked updates.
    if not all_projects:
        try:
            all_projects = sheets.get_all_projects()
        except Exception as exc:
            logger.warning("Could not load projects for score-based drafting: %s", exc)
            all_projects = []
    ...
    for project in all_projects:
        ...
        if name_key in seen_projects_today:
            continue  # already drafted above

        try:
            score = float(project.get("Score", 0) or 0)
        except (TypeError, ValueError):
            score = 0.0
        try:
        ...
        qualifies = score >= DRAFT_MIN_SCORE or confidence >= HIGH_CONFIDENCE_MIN
        if not qualifies:
            continue
        ...
        try:
            ...
            draft_text = generate_alpha_draft(project)
            sheets.add_draft({
                "Date":          datetime.utcnow().strftime("%Y-%m-%d"),
                "Project Name":  project_name,
                "Telegram Draft": draft_text,
            ...
            generated += 1
            seen_projects_today.add(name_key)
            logger.info("Score-based draft created for '%s' (score=%.1f).", project_name, score)
            time.sleep(0.5)

        except Exception as exc:
            logger.error("Failed to generate score-based draft for '%s': %s", project_name, exc)
            error_total += 1
