...
# ── Source: DeFiLlama Raises (disabled) ────────────────────────────────────

def discover_from_defillama_raises() -> list[dict]:
    """Disabled: DeFiLlama /raises now requires paid access and returns HTTP 402."""
    logger.info("DeFiLlama raises disabled (paid endpoint) — skipping source.")
    return []
...
def _passes_basic_quality(project: dict) -> bool:
    """Keep discovery broad: reject only obvious junk/dead projects."""
    name = str(project.get("Project Name", "")).strip()
    if not name or len(name) < 2:
        return False
    haystack = f"{name} {project.get('Description','')}".lower()
    if any(kw in haystack for kw in getattr(config, "MEME_KEYWORDS", set())):
        return False
    if any(kw in haystack for kw in getattr(config, "DEAD_PROJECT_KEYWORDS", set())):
        return False
    return True
