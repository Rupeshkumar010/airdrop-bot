...
DEFILLAMA_RAISES_WINDOW_DAYS = getattr(config, "DEFILLAMA_RAISES_WINDOW_DAYS", 180)

PREFERRED_CATEGORY_KEYWORDS = getattr(config, "PREFERRED_CATEGORY_KEYWORDS", (
    "l2", "layer 2", "rollup", "zk", "infrastructure", "infra", "ai",
    "rwa", "real world asset", "depin", "gaming", "game", "privacy",
    "restaking", "modular", "data availability", "oracle",
))

# Common corporate/branding suffixes stripped for fuzzy duplicate matching.
_NAME_NOISE_SUFFIXES = (
    " protocol", " finance", " network", " labs", " foundation",
    " chain", " app", " io", " xyz", " official",
)

_SOCIAL_LINK_PATTERNS = {
    "Twitter": (r"(?:https?://)?(?:www\.)?(?:twitter\.com|x\.com)/([A-Za-z0-9_]+)", "twitter/x"),
    "Discord": (r"(?:https?://)?(?:www\.)?discord\.(?:gg|com)/[A-Za-z0-9_/-]+", "discord"),
    "GitHub":  (r"(?:https?://)?(?:www\.)?github\.com/[A-Za-z0-9_.-]+(?:/[A-Za-z0-9_.-]+)?", "github"),
    "Docs":    (r"(?:https?://)?(?:docs\.[A-Za-z0-9_.-]+|[A-Za-z0-9_.-]+/docs)[A-Za-z0-9_/.-]*", "docs"),
}
...
def _enrich_project(project: dict) -> dict:
    """
    Best-effort auto-enrichment: if a project has a Website but is missing
    Twitter/Discord/GitHub/Docs, scrape the homepage once and try to pull
    those links out of it. Never raises — enrichment failures just leave
    the field blank, they never fail the whole discovery run.

    Also stamps `_preferred_category` (bool) onto the project when its
    category/description matches one of PREFERRED_CATEGORY_KEYWORDS, so
    scoring/reasons can give it a nudge.
    """
    haystack = f"{project.get('Category', '')} {project.get('Description', '')}".lower()
    project["_preferred_category"] = any(kw in haystack for kw in PREFERRED_CATEGORY_KEYWORDS)

    website = str(project.get("Website", "")).strip()
    needs = {
        "Twitter": not str(project.get("Twitter", "")).strip(),
        "Discord": not str(project.get("Discord", "")).strip(),
        "GitHub":  not str(project.get("GitHub", "")).strip(),
        "Docs":    not str(project.get("Docs", "")).strip(),
    }
    if not website or not website.startswith("http") or not any(needs.values()):
        return project
    ...
    try:
        ...
        for field, (pattern, _label) in _SOCIAL_LINK_PATTERNS.items():
            ...
            if field == "Twitter":
                ...
                if handle.lower() not in ("share", "intent", "home"):
                    project["Twitter"] = f"@{handle}"
            elif field in ("Discord", "GitHub", "Docs"):
                if not value.startswith("http"):
                    value = "https://" + value
                project[field] = value

    except Exception as exc:
        logger.debug("Enrichment failed for %s: %s", project.get("Project Name"), exc)

    return project


def _normalize_name(name: str) -> str:
    """Normalize a project name for fuzzy duplicate matching."""
    n = str(name or "").strip().lower()
    n = re.sub(r"[^a-z0-9 ]", "", n)
    for suffix in _NAME_NOISE_SUFFIXES:
        if n.endswith(suffix.strip()):
...
def _score_and_attach(project: dict, extra: Optional[dict] = None) -> dict:
    """Enrich, then run the project through scorer.calculate_score() and attach results."""
    _enrich_project(project)

    extra = dict(extra or {})
    extra.setdefault("website", project.get("Website", ""))
    extra.setdefault("github", project.get("GitHub", ""))
    extra.setdefault("docs", project.get("Docs", ""))
    extra.setdefault("discord", project.get("Discord", ""))
    if project.get("_preferred_category") and not extra.get("stage"):
        extra["stage"] = extra.get("stage") or "early"
...
def discover_from_defillama_raises() -> list[dict]:
    ...
    Includes raises from the last DEFILLAMA_RAISES_WINDOW_DAYS days
    (default 180 — widened from the original 30-day window since most
    good, fundable pre-airdrop projects raise well before they run
    testnets/campaigns).
    """
    logger.info("Fetching DeFiLlama raises (last %d days)...", DEFILLAMA_RAISES_WINDOW_DAYS)
    ...
    try:
        ...
        cutoff = datetime.utcnow() - timedelta(days=DEFILLAMA_RAISES_WINDOW_DAYS)
        found  = []
...
def run_discovery() -> None:
    ...
    # Fuzzy duplicate detection: normalize existing sheet names once up front
    # (catches "Monad" vs "Monad Labs" / "Monad Protocol", not just exact matches).
    existing_normalized: set[str] = set()
    try:
        for existing in sheets.get_all_projects():
            existing_normalized.add(_normalize_name(existing.get("Project Name", "")))
    except Exception as exc:
        logger.warning("Could not pre-load existing project names for fuzzy dedup: %s", exc)

    # Deduplicate by name and save to sheet
    seen_names: set[str] = set()      # exact, within this run
    seen_normalized: set[str] = set()  # fuzzy, within this run

    for project in all_projects:
        name = str(project.get("Project Name", "")).strip()
        norm = _normalize_name(name)

        if not name or name.lower() in seen_names or norm in seen_normalized:
            skipped_total += 1
            continue
        ...
        try:
            if norm in existing_normalized or sheets.project_exists(name):
            ...
            # Notify only for Score >= 7 (spec) — checked on the score itself,
            # not the tier label, so it stays correct even if tier cutoffs change.
            if scorer.meets_notify_threshold(project.get("Score", 0)):
                project["ID"] = pid
