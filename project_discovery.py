...
Sources used:
  1. DeFiLlama /protocols — public protocol metadata and TVL
  2. Airdrops.io          — public airdrop listing pages
  3. ICO Drops            — public ICO/project listings
  4. RootData             — optional, only when ROOTDATA_API_KEY is configured
  5. CryptoRank           — optional, only when CRYPTORANK_API_KEY is configured
  6. CoinMarketCap        — optional, only when CMC_API_KEY is configured

IMPORTANT:
  DeFiLlama /raises is intentionally disabled because the endpoint currently
  returns HTTP 402 Payment Required in GitHub Actions. The discovery run must
...
def discover_from_airdrops_io() -> list[dict]:
    ...
    try:
        ...
        for card in cards[:10]:  # limit to first 30 listings
            name_el = (
...
def run_discovery() -> None:
    ...
    for fn in source_fns:
        try:
            logger.info("Running discovery source: %s", fn.__name__)
            results = fn() or []
            all_projects.extend(results)
            found_total += len(results)
            logger.info("Source %s returned %d candidates.", fn.__name__, len(results))
        except Exception as exc:
