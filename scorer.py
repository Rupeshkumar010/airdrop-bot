...
def meets_notify_threshold(score_or_tier) -> bool:
    """
    Helper for notifier.py / project_discovery.py: should Telegram actually
    be notified? Spec: only notify Score >= NOTIFY_MIN_SCORE (default 7).

    Backward compatible: still accepts the old tier-string form
    ("Tier 1" / "Elite" / etc.) for any existing caller that passes a tier
    instead of a numeric score.
    """
    min_score = getattr(config, "NOTIFY_MIN_SCORE", 7.0)

    if isinstance(score_or_tier, str):
        # Old-style tier call — keep previous behavior for compatibility.
        return score_or_tier in ("Tier 1", "Elite")

    try:
        return float(score_or_tier) >= min_score
