...
For each update found it:
  - Classifies the update type  (Announcement / TGE / Snapshot / Launch / Warning)
...
def detect_update_type(text: str) -> str:
    """
    Return one of: TGE | Snapshot | Launch | Warning | Announcement
...
def check_twitter_for_updates(project: dict) -> list[dict]:
    ...
    Returns a list of update dicts ready to be saved.
    """
    if not config.TWITTER_BEARER_TOKEN:
    ...
    for tweet in tweets:
        ...
        # Only save Medium and above to keep the sheet clean
        if importance == "Low":
            continue
    ...
    logger.info(
        "Twitter check for '%s': found %d relevant tweets.",
...
def check_website_for_updates(project: dict) -> list[dict]:
    ...
    try:
        ...
        text = re.sub(r"\s+", " ", f"{title} {meta_desc} {text}")[:12000]
    ...
    importance  = detect_importance(text)
    if importance == "Low" and any(kw in text.lower() for kw in ("airdrop", "testnet", "quest", "points", "campaign")):
        importance = "Medium"
    update_type = detect_update_type(text)

    # Record Medium/High, and also recognize active airdrop/quest pages as Medium.
    # This makes the tracker useful even when Twitter API access is unavailable.
    if importance == "Low":
        return []
...
def _extract_key_sentence(text: str) -> str:
    ...
    # For active airdrop/testnet pages, create one useful baseline update so
    # the Updates -> Drafts pipeline has data even without Twitter API access.
    fallback = next((s for s in sentences if len(s.strip()) > 40), "")
    return summarise_text(fallback) if fallback else ""
...
def run_tracker() -> None:
    ...
    """
    logger.info("═══ Starting Update Tracker ═══")
    ...
    for project in active_projects:
        ...
        # Save and notify for each update
